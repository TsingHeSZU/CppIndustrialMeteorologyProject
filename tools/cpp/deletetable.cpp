...
// 程序帮助文档
void Help() {
    printf("Using: /CppIndustrialMeteorologyProject/tools/bin/deletetable logfilename xmlbuffer\n");
    printf("Sample:\n"\
        "/CppIndustrialMeteorologyProject/tools/bin/procctl 3600 "\
        "/CppIndustrialMeteorologyProject/tools/bin/deletetable "\
        ...
        "<starttime></starttime>"\
        "<timeout>120</timeout>"\
        "<pname>deletetable_ZHOBTMIND1</pname>\"\n");
    ...
    printf("starttime: 程序运行的时间区间, 例如 [02, 13] 表示: 如果程序运行时, 在 02 时到 13 时之间则运行, 其它时间不运行,"\
        "如果 starttime 为空, 本参数将失效, 只要本程序启动就会执行数据清理, "\
...
bool deleteTable() {
    ...
    char keyvalues[starg.maxcount][21];     // 存放唯一键字段值的数组

    sqlstatement stmt_delete(&conn);
    stmt_delete.prepare(deletesql);
    ...
    int ccount = 0;         // keyvalues 数组中有效元素的个数，绑定 delete 语句的输入
    ...
    if (stmt_select.rpc() > 0) {
        logfile.write("delete from %s %d rows in %.02f sec.\n", starg.tname, stmt_select.rpc(), timer.elapsed());
    }
