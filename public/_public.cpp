...
namespace idc
{
    ...
    // 初始化成员变量
    Cpactive::Cpactive() {
        printf("create cpactive.\n");
        this->m_shm_id = -1;        // 共享内存的引用 id
        this->m_shm = nullptr;      // 指向共享内存的地址空间
        this->m_pos = -1;           // 当前进程在共享内存进程组中的位置
    }

    // 把当前进程信息加入到共享内存进程组中
    bool Cpactive::addProcInfo(const int timeout, const string& pname, clogfile* logfile) {
        if (m_pos != -1) return true;

        // 创建/获取共享内存
        this->m_shm_id = shmget((key_t)SHMKEYP, MAXNUMP * sizeof(StProcInfo), 0666 | IPC_CREAT);
        if (this->m_shm_id == -1) {
            if (logfile != nullptr) {
        ...
        this->m_shm = (StProcInfo*)shmat(this->m_shm_id, NULL, 0);

        // 把当前进程的信息填充到结构体中
        StProcInfo proc_info(getpid(), pname.c_str(), timeout, time(0));
        ...
        */
        for (int i = 0;i < MAXNUMP;++i) {
            if ((this->m_shm + i)->pid == proc_info.pid) {
                this->m_pos = i;
                // printf("找到旧位置: i = %d\n", i);
                break;
            }
        ...
        if (sem_lock.init(SEMKEYP) == false) {   // 初始化信号量
            if (logfile != nullptr) {
        ...
        // 如果 m_pos == -1，表示共享内存的进程组中不存在当前进程编号，那就找一个空位置
        if (this->m_pos == -1) {
            // 在共享内存中寻找一个空的位置
            for (int i = 0;i < MAXNUMP;++i) {
                if ((this->m_shm + i)->pid == 0) {    // 创建共享内存时，内存中的数据默认初始化为 0
                    this->m_pos = i;
                    // printf("找到新位置: i = %d\n", i);
                    break;
                }
        ...
        // 把当前进程的结构体信息保存到共享内存中
        memcpy(this->m_shm + this->m_pos, &proc_info, sizeof(StProcInfo));
    ...
    // 更新共享内存进程组中，当前进程的心跳时间
    bool Cpactive::updateAtime() {
        if (this->m_pos == -1) {
            return false;
        }
    ...
    Cpactive::~Cpactive() {
        // 从共享内存中删除当前进程的心跳信息
        if (this->m_pos != -1) {
            printf("delete proc_info from shm.\n");
            memset(this->m_shm + this->m_pos, 0, sizeof(StProcInfo));   // 初始化内存空间为 0
        }
