...
#define SEMKEYP     0x5095     // 信号量的key
    ...
    class Cpactive {
    private:
        int m_shm_id;  // 共享内存的引用 id
        StProcInfo* m_shm;    // 指向共享内存的地址空间
        int m_pos;     // 存放当前进程在共享内存中，数组下标的位置
    ...
    public:
        ...
        // 把当前进程信息加入到共享内存进程组中
        bool addProcInfo(const int timeout, const string& pname = "", clogfile* logfile = nullptr);
